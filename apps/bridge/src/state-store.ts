...
function sanitizePendingSnapshot(value: PendingCurrentSessionsSnapshot | undefined): PendingCurrentSessionsSnapshot | undefined {
  ...
  const metadataOnly = exactKeys(candidate, ['request', 'digest', 'contentDigest'])
    && typeof candidate.digest === 'string' && typeof candidate.contentDigest === 'string'
    && request !== undefined && exactKeys(request, ['hostId', 'revision', 'observedAt', 'recipientSetVersion', 'sessions'])
    && typeof request.hostId === 'string' && typeof request.observedAt === 'string'
    && Number.isSafeInteger(request.revision) && (request.revision as number) > 0
    && Number.isSafeInteger(request.recipientSetVersion) && (request.recipientSetVersion as number) > 0
    && Array.isArray(sessions) && sessions.every((session) => {
      if (!session || typeof session !== 'object') return false;
      const member = session as Record<string, unknown>;
