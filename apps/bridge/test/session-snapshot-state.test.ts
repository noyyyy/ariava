...
describe('Bridge current session snapshot state', () => {
  test('persists immutable pending request and monotonic revision across restart', async () => {
    ...
    const first = await store.stageCurrentSessionsSnapshot('host-1', [makeSession()], 1, '2026-07-20T00:00:01.000Z');
    ...
    const second = await reloaded.stageCurrentSessionsSnapshot('host-1', [makeSession('working')], 1, '2026-07-20T00:00:02.000Z');
  ...
  test('does not allocate for accepted identical semantic content and honors stale lower bounds', async () => {
    ...
    expect(await store.stageCurrentSessionsSnapshot('host-1', [makeSession()], 1, '2026-07-20T00:00:10.000Z')).toBeUndefined();

    store.noteCurrentSessionsSnapshotRevisionLowerBound(8);
    const next = await store.stageCurrentSessionsSnapshot('host-1', [makeSession('working')], 1, '2026-07-20T00:00:11.000Z', 8);
  ...
  test('matching acceptance cannot clear a newer coalesced pending snapshot', async () => {
    ...
    const store = new BridgeStateStore(join(root, 'state.json'));
    const first = (await store.stageCurrentSessionsSnapshot('host-1', [makeSession()], 1, '2026-07-20T00:00:01.000Z'))!;
    const latest = (await store.stageCurrentSessionsSnapshot('host-1', [makeSession('working')], 1, '2026-07-20T00:00:02.000Z'))!;
  ...
  test('recipient version alone allocates a higher lifecycle revision', async () => {
    ...
    const first = (await store.stageCurrentSessionsSnapshot('host-1', [makeSession()], 1, '2026-07-20T00:00:01.000Z'))!;
    expect(store.acceptCurrentSessionsSnapshot(first.request.revision, first.digest)).toBe(true);
    const changed = await store.stageCurrentSessionsSnapshot('host-1', [makeSession()], 2, '2026-07-20T00:00:02.000Z');
