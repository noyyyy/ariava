...
The Codex Exact-Release capability PoC lives under `apps/bridge/test/codex-poc/` and `scripts/codex-poc/`. It is a research harness only: it never touches production `apps/bridge/src/**`, `apps/relay/**`, or `apps/watchos/**`, adds no production Codex CLI/service/manifest wiring, and its evidence is written only to the gitignored `.artifacts/` directory. Real experiments are strictly opt-in (`ARIAVA_CODEX_POC_OPT_IN=1`), operator-invoked, never run by ordinary lanes or CI, and report UNAVAILABLE (typically INCONCLUSIVE) when preconditions are missing instead of skipping. `codex:poc:run-desktop` additionally refuses `CI` / `GITHUB_ACTIONS`. Desktop PoC uses the current default Codex home and an explicit `--candidate` (on-disk bundle `ChatGPT.app`). The `codex:poc:inspect|run|review` scripts are the inventory/real-run/offline-review boundaries; they are dev-only and excluded from the published npm package.
