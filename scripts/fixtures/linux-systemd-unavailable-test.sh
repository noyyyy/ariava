...
export XDG_RUNTIME_DIR="$TEST_TMPDIR/no-user-bus"
...
for (const [name, expected] of [
  ...
  const output = fs.readFileSync(`${process.env.ARIAVA_SYSTEMD_TEST_TMPDIR}/${name}.json`, 'utf8') || fs.readFileSync(`${process.env.ARIAVA_SYSTEMD_TEST_TMPDIR}/${name}.err`, 'utf8');
...
const status = JSON.parse(fs.readFileSync(process.env.ARIAVA_SYSTEMD_TEST_TMPDIR + '/unavailable-status.json', 'utf8'));
